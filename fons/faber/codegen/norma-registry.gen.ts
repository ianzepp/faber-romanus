 ...
 * Generated: 2026-01-07T02:43:59.405Z
 */
    ...
    ['aleator', {
        ...
        methods: new Map([
            ...
            ['selige', {
                name: 'selige',
                translations: new Map([
                    ['ts', { template: '§[Math.floor(Math.random() * §.length)]', params: ['lista'] }],
                    ['py', { template: 'random.choice(§)', params: ['lista'] }],
                    ['rs', { template: '§.choose(&mut rand::thread_rng()).unwrap().clone()', params: ['lista'] }],
                    ['cpp', { template: '[&]{ std::random_device rd; std::mt19937 gen(rd()); std::uniform_int_distribution<size_t> dis(0, §.size() - 1); return §[dis(gen)]; }()', params: ['lista'] }],
                    ['zig', { template: 'blk: { var seed: u64 = undefined; std.crypto.random.bytes(std.mem.asBytes(&seed)); var prng = std.rand.DefaultPrng.init(seed); break :blk §[prng.random().uintLessThan(usize, §.len)]; }', params: ['lista'] }],
                ]),
            }],
            ['misce', {
                name: 'misce',
                translations: new Map([
                    ['ts', { template: '(() => { const a = [...§]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; })()', params: ['lista'] }],
                    ['py', { template: 'random.sample(§, len(§))', params: ['lista'] }],
                    ['rs', { template: '{ let mut v = §.clone(); v.shuffle(&mut rand::thread_rng()); v }', params: ['lista'] }],
                    ['cpp', { template: '[&]{ auto v = §; std::random_device rd; std::mt19937 gen(rd()); std::ranges::shuffle(v, gen); return v; }()', params: ['lista'] }],
